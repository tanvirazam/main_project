...
if($after_accos['status']==1){
    $select="SELECT COUNT( * ) as rrexist FROM service WHERE status=0";
    $select_res=mysqli_query($db_connection,$select);
    $after_accos=mysqli_fetch_assoc($select_res);
   if($after_accos['rrexist'] >=4){
    ...
    $update2="UPDATE service SET status=0 WHERE id=$id";
    ...
    $select="SELECT COUNT( * ) as exist FROM service WHERE status=1";
    $select_res=mysqli_query($db_connection,$select);
    $after_accos=mysqli_fetch_assoc($select_res);
